...
export async function apiPost<T>(
  ...
  // Add app_source for SheMax
  const dataWithSource = data instanceof FormData
    ? data
    : { ...data, app_source: 'shemax' };

  // Try direct Supabase first (except for FormData which needs HTTP)
  if (!(data instanceof FormData)) {
    const directResult = await tryDirectSupabase<T>(endpoint, 'POST', dataWithSource);
    if (directResult !== null) {
      return directResult;
    }
  }
  ...
  const body = isFormData ? data : (dataWithSource ? JSON.stringify(dataWithSource) : undefined);
